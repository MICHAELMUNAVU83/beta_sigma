defmodule BetaSigmaWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="fixed inset-0 bg-black/20 transition-opacity" aria-hidden="true" />
      <div
  ...
  attr :textarea_class, :string, default: nil, doc: "extra classes appended to a textarea input"

  attr :rest, :global,
    include: ~w(accept autocomplete capture cols disabled form list max maxlength min minlength
                multiple pattern placeholder readonly required rows size step phx-debounce phx-throttle
                phx-hook phx-update phx-target data-mention-users)
  ...
  def input(%{type: "textarea"} = assigns) do
    assigns = assign_new(assigns, :textarea_class, fn -> nil end)
  ...
  ## Examples
      ...
      <.avatar user={@current_user} class="h-8 w-8 text-xs" />
  """
  attr :user, :any, required: true
  attr :class, :string, default: "h-8 w-8 text-xs"
  ...
  def avatar(assigns) do
    ...
    <div class={[
      ...
      <img
        :if={is_map(@user) && Map.get(@user, :avatar_url)}
        src={@user.avatar_url}
        alt={"#{Map.get(@user, :name, Map.get(@user, :email, ""))} avatar"}
        class="h-full w-full object-cover"
      />
      <span :if={!is_map(@user) || !Map.get(@user, :avatar_url)}>
  ...
  defp avatar_initials(user) do
    name = Map.get(user, :name) || Map.get(user, :email) || ""
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BetaSigmaWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BetaSigmaWeb.Gettext, "errors", msg, opts)
    end
  end
